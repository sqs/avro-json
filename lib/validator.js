...
function makeFullyQualifiedTypeName(schema) {
  if (_.isString(schema)) {
    return schema;
  } else if (_.isObject(schema)) {
    if (_.isString(schema.type)) {
      return schema.type;
    } else if (_.isString(schema.namespace)) {
      return schema.namespace + '.' + schema.name;
    } else if (_.isUndefined(schema.namespace)) {
      return schema.name;
    } else {
      throw new InvalidSchemaError('namespace must either be undefined or string');
    }
  } else {
    throw new InvalidSchemaError('unable to determine fully qualified type name from schema ' + JSON.stringify(schema));
  }
}

function Union(typeSchemas) {
  this.branchNames = function() {
    return _.map(typeSchemas, function(typeSchema) { return makeFullyQualifiedTypeName(typeSchema); });
  };
...
function Validator(schema) {
  ...
  var _parseSchema = function(schema, parentSchema) {
    if (_.isNull(schema) || _.isUndefined(schema)) {
      ...
      if (schema.type === 'record') {
        ...
        return new Primitive(schema.type);
      } else {
        throw new InvalidSchemaError('not yet implemented: ' + schema.type);
      }
      ...
      var branchTypes = _.map(schema, function(branchType) { return _parseSchema(branchType, schema); });
      return new Union(branchTypes);
    } else {
