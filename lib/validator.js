...
function makeFullyQualifiedTypeName(namespace, name) {
  if (_.isString(namespace)) {
    return namespace + '.' + name;
  } else if (_.isUndefined(namespace)) {
    return name;
  } else {
  ...
  this.branchNames = function() {
    return _.map(typeSchemas, function(typeSchema) {
      if (_.isString(typeSchema)) {
        ...
        return makeFullyQualifiedTypeName(typeSchema.namespace, typeSchema.name);
      }
    });
  };
    ...
    if (_.isObject(obj)) {
      if (_.isArray(obj)) {
        ...
        if (_.contains(schema.branchNames(), unionBranch)) {
          ...
          throw new ValidationError('Expected union branch to be one of ' + JSON.stringify(schema.types) + '; got ' + JSON.stringify(unionBranch));
        }
      }
      ...
      if (_.contains(schema.branchNames(), 'null')) {
        ...
        throw new ValidationError('Expected union branch to be one of ' + JSON.stringify(schema.types) + '; got ' + JSON.stringify(obj));
      }
    } else {
